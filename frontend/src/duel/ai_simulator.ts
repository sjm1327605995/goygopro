...
import {
  POS_FACEUP_ATTACK, POS_FACEUP_DEFENSE, POS_FACEDOWN_DEFENSE,
  TYPE_MONSTER, TYPE_SPELL, TYPE_TRAP,
  LOC_HAND, LOC_MZONE, LOC_SZONE, LOC_GRAVE,
} from '../domain/constants.ts';

// 位置/类型常量的唯一来源见 domain/constants.ts
...
export class AISimulator {
  ...
  turn = 1;
  turnPlayer = 0;
  DELAY = 600; // pacing between engine steps; smoke tests shrink this
  currentPhase = 0x01;

  playerState: SideState = { lp: 0, deck: [], hand: [], mzone: [], szone: [], grave: [] };
  aiState: SideState = { lp: 0, deck: [], hand: [], mzone: [], szone: [], grave: [] };

  // Memoized card info so prompt building stays synchronous after preload.
  _cardInfo = new Map<number, any>();
  _onAction = (payload: PlayerAction) => this.handleAction(payload);
  ...
  // Halts the step chain (every scheduled step re-checks `running`). Without
  // this, leaving a practice duel mid-game leaves the AI ticking in the
  // background and a re-entered duel can never start (start() early-returns
  ...
  // Memoized card info so prompt building stays synchronous after preload.
  async info(code: number): Promise<any> {
    if (!code) return null;
  ...
  isMonster(info: any): boolean { return !!(info && (info.type & TYPE_MONSTER)); }
  isSpell(info: any): boolean { return !!(info && (info.type & TYPE_SPELL)); }
  isTrap(info: any): boolean { return !!(info && (info.type & TYPE_TRAP)); }

  state(player: number): SideState { return player === 0 ? this.playerState : this.aiState; }

  freeSlot(arr: (unknown | null)[]): number { return arr.findIndex((s) => s === null); }
  ...
  handleAction({ type, data }: PlayerAction): void {
  ...
  playerSummon(code: number, pos: number, isSpecial: boolean): void {
    ...
    // The real engine emits MSG_MOVE (hand → field) before the summoning
    // announcement; DuelManager relies on the move to drop the 2D hand card.
    eventBus.emit('duel:move', {
      code, pc: 0, pl: LOC_HAND, ps: handIdx, pp: pos,
      cc: 0, cl: LOC_MZONE, cs: slot, cp: pos
    });
    if (isSpecial) {
  ...
  playerSetSpellTrap(code: number): void {
    const st = this.playerState;
    const handIdx = st.hand.indexOf(code);
    if (handIdx === -1) return;
    ...
    eventBus.emit('duel:move', {
      code, pc: 0, pl: LOC_HAND, ps: handIdx, pp: 0xa,
      cc: 0, cl: LOC_SZONE, cs: slot, cp: 0xa
    });
    eventBus.emit('duel:set', { code, cc: 0, cl: LOC_SZONE, cs: slot, cp: 0xa });
    this.delay(300).then(() => this.promptIdle());
  }

  async playerActivate(code: number): Promise<void> {
    const st = this.playerState;
    const handIdx = st.hand.indexOf(code);
    if (handIdx === -1) return;
    const slot = this.freeSlot(st.szone);
    ...
    eventBus.emit('duel:move', {
      code, pc: 0, pl: LOC_HAND, ps: handIdx, pp: POS_FACEUP_ATTACK,
      cc: 0, cl: LOC_SZONE, cs: slot, cp: POS_FACEUP_ATTACK
    });
  ...
  playerReposition(seq: number): void {
  ...
  battleValue(entry: MonEntry): { val: number; isDef: boolean } {
  ...
  destroyCard(player: number, loc: number, seq: number): void {
    const st = this.state(player);
    if (loc === LOC_MZONE) {
  ...
  applyDamage(player: number, amount: number): void {
    if (amount <= 0) return;
    const st = this.state(player);
    st.lp -= amount;
  ...
  resolveBattle(attackerPlayer: number, attackerSeq: number): void {
    if (!this.running) return;
  ...
  async aiMainPhase(): Promise<void> {
